...
UserController extends BaseApiController {
	...
	public Map<String, Object> login(@RequestParam String uname, @RequestParam String password, HttpSession session) {
		...
		Password pwd = Password.decrypt(password, session);
	...
	public Map<String, Object> register(@RequestParam String uname, @RequestParam String realname, @RequestParam String password,
		...
		if (userService.selectByUname(uname.trim()) != null) return onBadResp(uname.trim() + " 已经存在，不能重复注册");
	...
	public Map<String, Object> list(@RequestParam(defaultValue = "true") Boolean authc, String keyword,
									@RequestParam(defaultValue = "1") Integer page_num, @RequestParam(defaultValue = "10") Integer page_size, HttpServletRequest request) {
		...
		switch (currentUser.getRoleType()) {
			...
			case SUBJECT_HOST:
				...
				return onDataResp(new MyPageInfo <User> (userService.list(keyword, null, authc, new Long[]{currentUser.getUnit().getId()}, new RoleTypeEnum[]{RoleTypeEnum.SUBJECT_HOST})));
		}
		return onDataResp(new MyPageInfo <User> (new ArrayList<>()));
	}
	...
	public Map<String, Object> listExpert(){
		return onDataResp(userService.list(null, null, null, null, new RoleTypeEnum[]{RoleTypeEnum.EXPERT}));
	}
	...
	@PostMapping("/authc")
	public Map<String, Object> authc(@RequestParam Long id) {
		User user = new User();
		user.setId(id);
		user.setActive(true);
		user.setAuthc(true);
	...
	public Map <String, Object>updatePwd (@RequestParam String password, HttpServletRequest request) {
		Password pwd = Password.decrypt(password, request.getSession());
		if (pwd == null) return onBadResp("密码错误");

		if (pwd.getPassword() == null || pwd.getPassword().trim().length() == 0) return onBadResp ("原密码长度必须大于零");
		if (pwd.getRePassword() == null || pwd.getRePassword().trim().length() == 0) return onBadResp ("新密码长度必须大于零");

		User user = userService.selectById(UserManager.getUser(request).getId());
		if (user == null || !user.getPassword().equals(Md5Utils.encrypt(user.getUname(), pwd.getPassword().trim()))) return onBadResp("密码修改失败");

		User updateUser = new User();
		updateUser.setId(user.getId());
		updateUser.setPassword(Md5Utils.encrypt(user.getUname(), pwd.getRePassword().trim()));
	...
	public Map<String,Object> delte(@RequestParam Long id) {
		List<com.zhuoyue.researchManement.bean.Subject> subjects = subjectService.list(id, null, null, null);
		if (subjects.size() > 0) return onBadResp("该账号存在相关课题，无法删除");

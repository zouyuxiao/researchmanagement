    ...
    public Map<String, Object> list(String keyword, Long subject_id, @RequestParam(defaultValue = "1") Integer page_num, @RequestParam(defaultValue = "10") Integer page_size, HttpServletRequest request)
    {
        ...
        switch (currentUser.getRoleType()) {
            case CITY_RESEARCH:
                ...
                return onDataResp( new MyPageInfo<Activity>(activityService.list(keyword, subject_id, null, null, new ActivityState[]{ActivityState.IN_CITY_CHECK})));
            case AREA_RESEARCH:
                ...
                PageHelper.startPage(page_num,page_size);
                return onDataResp( new MyPageInfo<Activity>(activityService.list(keyword, subject_id, null, unitIds, new ActivityState[]{ActivityState.IN_AREA_CHECK})));
            case SCHOOL_RESEARCH:
                PageHelper.startPage(page_num,page_size);
                return onDataResp( new MyPageInfo<Activity>(activityService.list(keyword, subject_id, null, new Long[]{currentUser.getUnit().getId()}, new ActivityState[]{ActivityState.IN_SCHOOL_CHECK})));
            case SUBJECT_HOST:
                PageHelper.startPage(page_num,page_size);
